...
HMotor::HMotor(int motor_id) {
	if(motor_id != MOTOR_LEFT && motor_id != MOTOR_RIGHT) { Serial.print("Bad ID"); }
	else {m_id = motor_id;
...
void HMotor::setDirection(int dir) {
	...
	switch(dir) {
		case MOTOR_FWD:
			...
			//Serial.println(_p_in_a);
			//Serial.println(_fwd_val[0]);
			digitalWrite(_p_in_b, _fwd_val[1]);
			//Serial.println(_p_in_b);
			//Serial.println(_fwd_val[1]);
			_dir = MOTOR_FWD;
			break;
		case MOTOR_BWD:
